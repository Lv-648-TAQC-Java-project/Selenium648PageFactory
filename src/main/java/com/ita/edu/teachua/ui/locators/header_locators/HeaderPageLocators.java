package com.ita.edu.teachua.ui.locators.header_locators;


public class HeaderPageLocators {
    public static final String CLUBS_LINK = "//a[contains(text(),'Гуртки')]";
    public static final String ABOUT_US_LINK = "//a[contains(text(),'Про нас')]";
    public static final String SERVICES_LINK_XPATH = "//a[contains(text(),'Послуги українською')]";
    public static final String GUEST_DROPDOWN_XPATH = "//div[@class = 'ant-dropdown-trigger user-profile']/span[@role]";
    public static final String OWNER_DROPDOWN_CSS_SELECTOR = "div.ant-dropdown-trigger.user-profile span.anticon.anticon-caret-down";
}
